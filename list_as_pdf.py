...
pdf = PdfPages('left_ventricle_groups.pdf')

for x in range(len(arr_of_groups)) :
    arr = arr_of_groups[x]
    
    
##    Max 6 diagrams per page. Initialise first page
##
    perPage = 6
    ...
    #fig.canvas.set_window_title('Left Ventricle Bulls Eyes (AHA) Group ' + str(x + 1) + ' page 1')
    plt.title('Left Ventricle Bulls Eyes (AHA) Group ' + str(x + 1) + ' page 1')
    ...
    for y in range (len(arr)):
        ...
        bullseye_plot(chain[chainCounter], data, vlim=vlim)
        chain[chainCounter].set_title('Patient ' + str(y+1))

        if(y % perPage == 5 or y == len(arr) - 1) :
            # save and move onto the next page
            ...
            chain[chainCounter] = fig.add_axes([0.1, 0.01, 0.1, 0.025])
                
            cb = mpl.colorbar.ColorbarBase(chain[chainCounter], cmap=cm, norm=cNorm, ticks=ticks,
                                               orientation='horizontal')
            
            pdf.savefig(fig)
            plt.close()
            print('Closing page ' + str(int(y/6) + 1))

            #reinitialise fig etc if there is a next page
            if(y != len(arr) - 1) :
                fig, ax = plt.subplots(figsize=(12, 8), nrows=2, ncols=3,
                                   subplot_kw=dict(projection='polar'))
                chain = list(itertools.chain.from_iterable(ax))
                
                plt.title('Left Ventricle Bulls Eyes (AHA) Group ' + str(x + 1) + ' page ' + str(int(y/6)+1))
                chainCounter = -1 # Not 0, because we are going to increment it immediately after this if-statement
        chainCounter += 1         
    ...
    plt.title('Average for Group ' + str(x + 1))
    data = arr_av(arr)

    bullseye_plot(ax, data, vlim=vlim)
    #ax.set_title("Average for Group " + str(x + 1))
    pdf.savefig(fig)
    plt.close()
    print('Closing average page '+ str(x + 1))
             
#plt.show()
d = pdf.infodict()
d['Title'] = 'Left Ventricle Diagrams'
d['Author'] = u'Author name here'
d['Subject'] = 'Put Subject Here'
d['Keywords'] = 'put keywords here'
print('Closing PDF... Please wait...')
pdf.close()
print('..Closed')
